...
The `./r` script sets up `PYTHONPATH` to include sibling project `lib/` directories (`bes`, `bav`, `bnet`) alongside this project's `lib/`. All runs must go through `./r` to have the correct path. Python version is pinned in `.python-version`.
