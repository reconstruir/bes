...
class test_caca_archive_unix_tar(unittest.TestCase, caca_caca_common_archive_tests):
  ...
  def __init__(self, methodName = 'runTest'):
    super(test_caca_archive_unix_tar, self).__init__(methodName)
    self.default_caca_archive_type = caca_archive_extension.TAR

  def make_caca_archive(self, filename):
    return caca_archive_unix_tar(filename)

  def test_init(self):
    self.assertEqual( 'foo.tar', caca_archive_unix_tar('foo.tar').filename )
  ...
  def test_file_is_valid(self):
    ...
    self.assertFalse( caca_archive_unix_tar.file_is_valid(tmp_zip.filename) )
    ...
    self.assertTrue( caca_archive_unix_tar.file_is_valid(tmp_tar.filename) )
    ...
    self.assertTrue( caca_archive_unix_tar.file_is_valid(tmp_tgz.filename) )

    tmp_xz = temp_caca_archive.make_temp_caca_archive([ temp_caca_archive.Item('foo.txt', content = 'foo.txt\n') ], caca_archive_extension.XZ)
    self.assertTrue( caca_archive_unix_tar.file_is_valid(tmp_xz.filename) )

    self.assertFalse( caca_archive_unix_tar.file_is_valid(temp_file.make_temp_file(content = 'junk\n')) )
