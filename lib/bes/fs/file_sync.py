  ...
  def sync(clazz, src_dir, dst_dir, exclude = None):
    ...
    for dst_file in dst_files:
      if not dst_file in src_files:
        bf_file_ops.remove(path.join(dst_dir, dst_file))
    ...
    for src_file in src_files:
      ...
      if path.isfile(dst_file_path):
        ...
        should_copy_mode = bf_entry(src_file_path).mode != bf_entry(dst_file_path).mode
      else:
        should_copy = True
        should_copy_mode = True
      if should_copy:
        bf_file_ops.copy(src_file_path, dst_file_path)
      if should_copy_mode:
        bf_file_ops.copy_mode(src_file_path, dst_file_path)
