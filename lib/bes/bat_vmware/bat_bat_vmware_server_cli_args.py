...
class bat_bat_vmware_server_cli_args(object):

  def __init__(self):
    pass
  
  def bat_vmware_server_add_args(self, subparser):

    # shell
    p = subparser.add_parser('shell', help = 'Run interactive vmrest shell.')
    self.__bat_vmware_server_add_common_args(p)
  ...
  def __bat_vmware_server_add_common_args(self, p):
    p.add_argument('-v', '--verbose', action = 'store_true', default = False,
                   help = 'Verbose output [ False ]')
    p.add_argument('-p', '--port', action = 'store', type = int, default = None,
                   help = 'Port [ 8697 ]')
    
  def _command_bat_vmware_server(self, command, *args, **kargs):
    from .bat_bat_vmware_server_cli_handler import bat_bat_vmware_server_cli_handler
    return bat_bat_vmware_server_cli_handler(kargs).handle_command(command)
