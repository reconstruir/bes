...
class bat_vmware_credentials(object):

  _log = logger('bat_vmware_credentials')
...
'''
  ...
  def set_credentials(clazz, username, password, num_tries = None):
    ...
    if num_tries < 1:
      raise bat_vmware_error('Num tries should be between 1 and 10: {}'.format(num_tries))
    if num_tries > 100:
      raise bat_vmware_error('Num tries should be between 1 and 10: {}'.format(num_tries))
  ...
  def _do_set_credentials(clazz, username, password, print_error):
    ...
    try:
      ...
      clazz._log.log_d('_do_set_credentials: exit_code={} stdout="{}" stderr="{}"'.format(rv.exit_code,
                                                                                          ...
                                                                                          rv.stderr))
      if rv.exit_code != 0:
        raise bat_vmware_error(clazz._EXPECT_SCRIPT_MSG_MAP[rv.exit_code])
    finally:
