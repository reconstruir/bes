...
class vmware_rest_cli_args(object):

  def __init__(self):
    pass
  
  def vmware_rest_add_args(self, subparser):
  ...
  def _command_vmware_rest(self, command, *args, **kargs):
    from .vmware_rest_cli_command import vmware_rest_cli_command
    return vmware_rest_cli_command(kargs).handle_command(command)
