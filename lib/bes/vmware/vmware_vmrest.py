...
class vmware_vmrest(with_metaclass(ABCMeta, object)):

  class handler(simple_server.WSGIRequestHandler):

    def __init__(self, request, client_address, server):
      log.add_logging(self, 'vmware_vmrest')
  ...
  def __init__(self, port = None, log_tag = None, users = None):
    log.add_logging(self, tag = log_tag or 'vmware_vmrest')
    self.log_i('vmware_vmrest(id={} self={}, port={})'.format(id(self), self, port))
