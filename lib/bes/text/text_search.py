...
class text_search(object):
  ...
  @classmethod
  def find_all_gen(clazz, text, sub_string, boundary_chars = None):
    'Yields all the spans containing sub_string in text using optional word boundary checking'
    ...
    while True:
      ...
      if boundary_chars != None:
        do_yield = word_boundary.word_has_boundary(text, start, end, chars = boundary_chars)
      else:
        do_yield = True
      if do_yield:
  ...
  @classmethod
  def find_all(clazz, text, sub_string, boundary_chars = None):
    'Returns a list of of all the spans containing sub_string in text using optional word boundary'
    check.check_string(text)
    check.check_string(sub_string)
    check.check_set(boundary_chars, allow_none = True)

    return [ span for span in clazz.find_all_gen(text, sub_string, boundary_chars = boundary_chars) ]
