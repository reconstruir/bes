...
from .btl_parser_desc_token import btl_parser_desc_token
from .btl_error import btl_error

class btl_parser_desc_token_list(type_checked_list):

  __value_type__ = btl_parser_desc_token
  ...
  def parse_node(clazz, n, source = '<unknown>'):
    ...
    result = btl_parser_desc_token_list()
    if not n:
      return result
    existing = set()
    for child in n.children:
      next_desc_token = btl_parser_desc_token.parse_node(child, source)
...
btl_parser_desc_token_list.register_check_class()
