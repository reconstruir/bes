...
from .btl_parser_desc_state import btl_parser_desc_state

class btl_parser_desc_state_list(type_checked_list):

  __value_type__ = btl_parser_desc_state
  ...
  def parse_node(clazz, n, source = '<unknown>'):
    ...
    result = btl_parser_desc_state_list()
    if not n:
      return result
    for child in n.children:
      next_desc_state = btl_parser_desc_state.parse_node(child, source = source)
  ...
  def generate_code(self, buf, errors):
    ...
    for state in self:
      state.generate_code(buf, errors)
