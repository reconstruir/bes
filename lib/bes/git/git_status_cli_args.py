...
class git_status_cli_args(object):

  def __init__(self):
    pass
  
  def git_status_add_args(self, subparser):

    # status
    p = subparser.add_parser('get', help = 'Print status for one or more git dirs.')
    self._git_status_add_common_args(p)
    p.add_argument('dirs', action = 'store', nargs = '*', help = 'Dirs to check for status.')
    
  @classmethod
  def _git_status_add_common_args(clazz, p):
  ...
  def _command_git_status(self, command, *args, **kargs):
    from .git_status_cli_handler import git_status_cli_handler
    return git_status_cli_handler(kargs).handle_command(command)
