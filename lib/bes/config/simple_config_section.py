...
class simple_config_section(object):
  ...
  def __init__(self, header, entries, origin, extends_section=None, parent_variables=None):
    ...
    check.check_simple_config_variables(parent_variables, allow_none=True)

    self.header = header
    self.entries = entries or []
    self.origin = origin
    self.extends_section = extends_section
    self.variables = simple_config_variables()
    self.parent_variables = parent_variables or simple_config_variables()
  ...
  def clone(self, mutations=None):
    result = simple_config_section(
      self.header,
      copy.deepcopy(self.entries),
      self.origin,
      extends_section=self.extends_section,
      parent_variables=copy.copy(self.parent_variables)
    )
    result.variables = copy.copy(self.variables)
    if mutations:
  ...
  def to_string(self, entry_formatter=None, sort=False, fixed_key_column_width=False):
    ...
    buf = StringIO()
    buf.write(str(self.header))
    buf.write('\n')
    entries = self.entries if not sort else sorted(self.entries)
    key_column_width = 0
  ...
  def find_by_key(self, key, raise_error=True, resolve_env_vars=True):
    ...
    if not entry and self.extends_section:
      entry = self.extends_section.find_entry(key)
    if not entry:
      if raise_error:
  ...
  def entry_index(self, key):
    for i, entry in list_util.reversed_enumerate(self.entries):
  ...
  def match_by_key(self, pattern, raise_error=True, resolve_env_vars=True):
    ...
    if not entry and self.extends_section:
      entry = self.extends_section.match_entry(pattern)
    if not entry:
      if raise_error:
        raise simple_config_error('"{}" entry not found'.format(pattern), self.origin)
      return None
    value = entry.value.value
    if resolve_env_vars:
      value = self._resolve_variables(value, entry.origin)
  ...
  def set_value(self, key, value, hints=None):
    ...
    for i, entry in enumerate(self.entries):
      if entry.value.key == key:
        found = True
        self.entries[i] = simple_config_entry(key_value(entry.value.key, value),
                                              origin=entry.origin,
  ...
  def to_key_value_list(self, resolve_env_vars=False):
    ...
    if self.extends_section:
      result.extend(self.extends_section.to_key_value_list(resolve_env_vars=resolve_env_vars))
  ...
  def to_dict(self, resolve_env_vars=True):
    ...
    result = {}
    if self.extends_section:
      result.update(self.extends_section.to_dict(resolve_env_vars=resolve_env_vars))
    for entry in self:
      if resolve_env_vars:
    ...
    for var in variables:
      ...
      if os_var.is_set:
        ...
        if var in self.variables:
          ...
          found = True
        elif var in self.parent_variables:
          value = self.parent_variables[var]
          found = True
      if not found:
  ...
  def update_variables(self, variables):
    ...
    self.variables.update_variables(variables)
