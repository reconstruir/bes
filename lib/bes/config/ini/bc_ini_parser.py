...
class bc_ini_parser(object):

  def __init__(self, options, delimiter, empty_value, log_tag):
    log.add_logging(self, tag = log_tag or 'bc_ini_parser')
  ...
  def _run(self, text):
    ...
    for token in lexer.tokenize(text, 'bc_ini_lexer', options = self._options):
  ...
  def parse(clazz, text, options = 0, delimiter = '=', empty_value = None, log_tag = None):
    return bc_ini_parser(options, delimiter, empty_value, log_tag)._run(text)
