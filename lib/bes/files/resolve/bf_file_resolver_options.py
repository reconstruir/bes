...
from .bf_file_resolver_progress import bf_file_resolver_progress
...
class bf_file_resolver_options(bcli_options):
  ...
  def _check_depth_limits(self):
    if self.max_depth and self.min_depth and not (self.max_depth >= self.min_depth):
      raise bf_file_resolver_error('max_depth needs to be >= min_depth.')

    if self.min_depth and self.min_depth < 1:
      raise bf_file_resolver_error('min_depth needs to be >= 1.')
  ...
  def call_stop_function(self):
    ...
    if not check.is_bool(result):
      raise bf_file_resolver_error(f'result from stop_function should be bool: "{result}" - {type(result)}')
    return result
  ...
  def call_progress_function(self, state, index, total):
    ...
    if not self.progress_function:
      return
    progress = bf_file_resolver_progress(state, index, total)
