...
class bf_file_resolver(object):
  ...
  def _resolve_gen_with_stats(self, where, stats_dict):
    where = bf_check.check_dir_seq(object_util.listify(where))
    for next_where in where:
      for entry in self._resolve_gen_one_dir(next_where, stats_dict):
        yield entry
        
  def _resolve_gen_one_dir(self, where, stats_dict):
    where = bf_check.check_dir(where)
    where = path.normpath(where)
    result = bf_entry_list()
    where = path.normpath(where)

    self._log.log_d(f'resolve_gen: where={where} options={self._options}')
  ...
  def resolve_with_stats(self, where):
    stats_dict = {
      'num_checked': 0,
      'num_files_checked': 0,
      'num_dirs_checked': 0,
    ...
    for entry in self._resolve_gen_with_stats(where, stats_dict):
      entries.append(entry)
    stats_dict['end_time'] = datetime.now()
    stats = bf_file_resolver_stats(stats_dict['num_checked'],
                                 stats_dict['num_files_checked'],
                                 stats_dict['num_dirs_checked'],
                                 stats_dict['start_time'],
                                 stats_dict['end_time'],
                                 stats_dict['depth'])
    return bf_file_resolver_result(entries, stats)
  ...
  @classmethod
  def resolve_with_options(clazz, where, **kwargs):
    options = bf_file_resolver_options(**kwargs)
    resolver = bf_file_resolver(options = options)
    return resolver.resolve(where).sorted_by_criteria('FILENAME')
  
  @classmethod
  def resolve_with_fnmatch(clazz, where,
    ...
    return clazz.resolve_with_options(where,
                                   file_type = file_type,
