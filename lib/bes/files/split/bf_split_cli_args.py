...
class bf_split_cli_args(object):

  def __init__(self):
    pass
  
  def bf_split_add_args(self, subparser):
    # unsplit
    p = subparser.add_parser('unsplit', help = 'Unsplit files.')
    self.__bf_split_cli_add_add_common_args(p)
    p.add_argument('files', action = 'store', default = [], nargs = '+',
                   help = 'One or more files or dirs to find dups in [ None ]')
    
  @classmethod
  def __bf_split_cli_add_add_common_args(clazz, p):
  ...
  def _command_bf_split(self, command, *args, **kargs):
    from .bf_split_cli_handler import bf_split_cli_handler
    return bf_split_cli_handler(kargs).handle_command(command)
