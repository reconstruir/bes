...
class uv_installer_linux(uv_installer_base.uv_installer_base):
  'uv installer for Linux using the official install script.'
  ...
  def install(self, version=None):
    ...
    if self.options.install_script:
      ...
      self._download_and_run_script(install_environment)

    exe = self.exe_path()
    if not exe:
      install_dir_str = self.options.resolve_install_dir()
      if not install_dir_str in os.environ.get('PATH', '').split(os.pathsep):
  ...
  def exe_path(self):
    install_dir = self.options.resolve_install_dir()
    candidate = path.join(install_dir, 'uv')
    if path.isfile(candidate):
      return candidate
    return None

  def _run_script(self, script_path, environment):
    execute.execute(['sh', script_path],
                    env=environment,
                    raise_error=True,
                    stderr_to_stdout=True,
                    check_python_script=False)

  def _download_and_run_script(self, environment):
    fd, tmp_path = tempfile.mkstemp(suffix='.sh')
    try:
      os.close(fd)
      execute.execute(['curl', '-LsSf', self._UV_INSTALL_URL, '-o', tmp_path],
                      raise_error=True,
                      stderr_to_stdout=True,
                      check_python_script=False)
      self._run_script(tmp_path, environment)
    finally:
