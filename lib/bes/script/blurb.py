...
class blurb(object):
  ...
  _CONFIG_FILENAME = path.expanduser('~/.bes_blurb_config')
  _DEFAULT_CONFIG = { 'label_length': 10 }
  ...
  def set_verbose(clazz, verbose):
    clazz._verbose = verbose

  @classmethod
  def set_label_length(clazz, label_length):
  ...
  @classmethod
  def _load_config(clazz):
    'Load blurb config from disk.'
    o = json_util.read_file(clazz._CONFIG_FILENAME)
    if not o:
      return clazz._DEFAULT_CONFIG
    assert 'label_length' in o
    return o

  @classmethod
  def _save_config(clazz, config):
    'Load blurb config from disk.'
    json_util.save_file(clazz._CONFIG_FILENAME, config, indent = 2)

  @classmethod
  def _get_label_length(clazz):
    if clazz._label_length == None:
      config = clazz._load_config()
