...
class shell_framework(object):
  ...
  def extract(clazz, where):
    dst_path = path.join(where, path.basename(clazz._BES_SHELL_DOT_SH_FILENAME))
    file_util.save(dst_path, content = clazz._BES_SHELL_DOT_SH_CONTENT, mode = 0o755)
    if file_util.read(dst_path, codec = 'utf8') != clazz._BES_SHELL_DOT_SH_CONTENT:
      raise RuntimeError('Failed to save %s.' % (dst_path))

  _BES_SHELL_DOT_SH_FILENAME = 'env/bes_shell.sh'
  _BES_SHELL_DOT_SH_CONTENT = r'''\
