...
class AsyncHandler(Thread):
  ...
  def run(self):
    ...
    while True:
      self.log_d('calling __main_loop()')
  ...
  def __main_loop(self):
    self.log_d('__main_loop() starting')
    self._running_lock.acquire()
    ...
    self.log_d('__main_loop() waiting for message')
    data = self._queue.get(block = True, timeout = None)
    self.log_d('__main_loop() got message')

    if data == None:
      self.log_d('__main_loop() got terminate')
      ...
      self.log_d('__main_loop() got data')
      self._handler(data)
      self._queue.task_done()
    return True
