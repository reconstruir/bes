...
class test_git_util(unit_test):
  ...
  def test_repo_last_tag(self):
    r = temp_git_repo()
    r.add_file('readme.txt', 'readme is good')
    r.push('origin', 'master')
    r.tag('1.0.0')
    r.push_tag('1.0.0')
    self.assertEqual( '1.0.0', git_util.repo_last_tag(r.address) )
    r.tag('1.0.1')
    r.push_tag('1.0.1')
    self.assertEqual( '1.0.1', git_util.repo_last_tag(r.address) )
  ...
  def test_repo_bump_tag(self):
    ...
    git_util.repo_bump_tag(r1.address, None, False)
    r2.pull()
    self.assertEqual( '1.0.2', r2.last_local_tag() )
  ...
  def test_repo_bump_tag_single_number(self):
    ...
    git_util.repo_bump_tag(r1.address, None, False)
    r2.pull()
    self.assertEqual( '667', r2.last_local_tag() )

    git_util.repo_bump_tag(r1.address, None, False)
    r2.pull()
    self.assertEqual( '668', r2.last_local_tag() )
