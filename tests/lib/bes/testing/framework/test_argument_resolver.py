...
class test_argument_resolver(unit_test):
  ...
  def test_dot_dir_water(self):
    ...
    self.assertEqual( 1, len(ar.files) )
    self.assertEqual( self.data_path('water/tests/lib/water/common/test_water_util.py'), ar.files[0].file_info.filename )
    
  def test_dot_dir_orange(self):
    ar = self._make_test_argument_resolver([ '.' ], working_dir = self.data_path('orange'))
    self.assertEqual( 1, len(ar.files) )
    self.assertEqual( self.data_path('orange/tests/lib/orange/common/test_orange_util.py'), ar.files[0].file_info.filename )
    
  def test_dot_dir(self):
    ar = self._make_test_argument_resolver([ '.' ], working_dir = self.data_dir())
    self.assertEqual( 2, len(ar.files) )
    self.assertEqual( self.data_path('orange/tests/lib/orange/common/test_orange_util.py'), ar.files[0].file_info.filename )
    self.assertEqual( self.data_path('water/tests/lib/water/common/test_water_util.py'), ar.files[1].file_info.filename )
