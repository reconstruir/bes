...
class test_file_split_command_handler(program_unit_test):
  ...
  def test_find_and_unsplit(self):
    ...
    t = self._find_and_unsplit_test(extra_content_items=items, recursive=True)
  ...
  def test_find_and_unsplit_dry_run(self):
    ...
    t = self._find_and_unsplit_test(extra_content_items=items, recursive=True, dry_run=True)
    self.assert_filename_list_equal([
      'a',
      'a/foo',
      'a/foo/kiwi.txt',
  ...
  def _find_and_unsplit_test(self,
    ...
    with dir_operation_tester(extra_content_items=extra_content_items) as test:
      ...
      if check_downloading:
        args.append('--downloading')
      if dry_run:
        args.append('--dry-run')
      args.extend(['unsplit', test.src_dir])
