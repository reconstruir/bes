...
class test_file_duplicates_command_handler(program_unit_test):
  ...
  def test_find_duplicates_delete(self):
    ...
    t = self._find_dups_test(extra_content_items=items, recursive=True, delete=True)
    self.assert_string_equal_fuzzy(
      f'{path.join(t.src_dir, "a", "kiwi.jpg")}:\n'
      f'  {path.join(t.src_dir, "b", "kiwi_dup1.jpg")}\n'
      f'  {path.join(t.src_dir, "c", "kiwi_dup2.jpg")}\n',
  ...
  def _find_dups_test(self,
    ...
    with dir_operation_tester(extra_content_items=extra_content_items) as test:
      args = ['file_duplicates']
      if recursive:
        args.append('--recursive')
      args.extend(['dups', test.src_dir])
      if delete:
        args.append('--delete')
      if keep_empty_dirs:
        args.append('--keep')
