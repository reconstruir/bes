...
class test_archive_tar(make_test_case(archive_tar, archive_extension.TAR)):
    ...
    def test_file_is_valid(self):
      ...
      self.assertTrue( archive_tar.file_is_valid(tmp_tar) )
  
      tmp_tgz = temp_archive.make_temp_archive([ temp_archive.item('foo.txt', content = 'foo.txt\n') ], archive_extension.TGZ)
      self.assertTrue( archive_tar.file_is_valid(tmp_tgz) )
  
      tmp_tar_gz = temp_archive.make_temp_archive([ temp_archive.item('foo.txt', content = 'foo.txt\n') ], archive_extension.TAR_GZ)
      self.assertTrue( archive_tar.file_is_valid(tmp_tar_gz) )
  
      tmp_zip = temp_archive.make_temp_archive([ temp_archive.item('foo.txt', content = 'foo.txt\n') ], archive_extension.ZIP)
      self.assertFalse( archive_tar.file_is_valid(tmp_zip) )
  
      self.assertFalse( archive_tar.file_is_valid(self.make_temp_file(content = 'junk\n')) )
