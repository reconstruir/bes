...
'''
    ...
    config_text = bf_file_ops.read(path.join(tmp_dir, 'config'), codec = 'utf-8').strip()
    expected_config = '''
Host bitbucket.org
...
'''.format(private_key = private_key)
    ...
    known_hosts_text = bf_file_ops.read(path.join(tmp_dir, 'known_hosts'), codec = 'utf-8').strip()
