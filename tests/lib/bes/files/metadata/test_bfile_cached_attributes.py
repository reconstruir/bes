...
class test_bfile_cached_attributes(unit_test, unit_test_media_files):
  ...
  def xtest_get_bytes_none_value(self):
    tmp = self.make_temp_file(content = 'this is foo', suffix = '.txt')
    def _value_maker(f):
      return None
  ...
  def xtest_get_bytes_with_change(self):
    tmp = self.make_temp_file(content = 'this is foo', suffix = '.txt')
  ...
  def xtest_get_mime_type_jpeg(self):
  ...
  def xtest_register_getter(self):
