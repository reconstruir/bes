...
class test_bfile_symlink(unit_test):

  @classmethod
  def setUpClass(clazz):
    unit_test_class_skip.raise_skip_if(bfile_symlink.has_support(), 'No support or priviledge for symlinks')
  ...
  def test_is_broken_true(self):
    ...
    self.assertEqual( True, bfile_symlink.is_broken(tmp.name) )
  ...
  def test_is_broken_false(self):
    ...
    self.assertEqual( False, bfile_symlink.is_broken(tmp1.name) )
  ...
  def test_resolve_one_level(self):
    ...
    self.assertEqual( tmp1, bfile_symlink.resolve(tmp1) )
    self.assertEqual( tmp1, bfile_symlink.resolve(tmp2) )
  ...
  def test_resolve_two_levels(self):
    ...
    self.assertEqual( tmp1, bfile_symlink.resolve(tmp1) )
    self.assertEqual( tmp1, bfile_symlink.resolve(tmp2) )
    self.assertEqual( tmp1, bfile_symlink.resolve(tmp3) )
