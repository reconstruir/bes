...
from bes.files.bf_mtime_cached_info import bf_mtime_cached_info
from bes.testing.unit_test import unit_test
  
class test_bf_mtime_cached_info(unit_test):

  def test_value(self):
    tmp = self.make_temp_file(content = 'kiwi')
    a = bf_mtime_cached_info(tmp, lambda f: path.getsize(f))
