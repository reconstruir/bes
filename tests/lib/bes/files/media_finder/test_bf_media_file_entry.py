...
class test_bf_media_file_entry(unit_test):
  ...
  def test_resolved_features_default_empty(self):
    e = self._make_entry()
    self.assertEqual({}, e.resolved_features)

  def test_resolved_features_mutable(self):
    e = self._make_entry()
    e.resolved_features['width'] = 1920
    self.assertEqual(1920, e.resolved_features['width'])

  def test_resolved_features_field_frozen(self):
    import dataclasses
    e = self._make_entry()
